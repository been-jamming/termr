...
void execute_action(unsigned char update_type){
	...
	switch(update_type){
		...
		case NEXT_FRAME:
			...
			if(current_nanoseconds - last_nanoseconds < 25000000ULL*frame_count){
				sleep_time = (struct timespec) {.tv_sec = (25000000ULL*frame_count - current_nanoseconds + last_nanoseconds)/1000000000ULL, .tv_nsec = (25000000ULL*frame_count - current_nanoseconds + last_nanoseconds)%1000000000ULL};
				nanosleep(&sleep_time, NULL);
				last_time.tv_sec = (last_nanoseconds + 25000000ULL*frame_count)/1000000000ULL;
				last_time.tv_nsec = (last_nanoseconds + 25000000ULL*frame_count)%1000000000ULL;
			} else {
